...
matrice_sym& matrice_sym::operator+=(const matrice_sym& A)
{
    ...
    for (int k=0; k<n; ++k)
    {
        if (Profil[k]!=A.Profil[k])
        {
            cout<<"Les matrices n'ont pas même profil"<<endl;
            exit(-1);
        }
    }
    for (int i=0; i<n; ++i)
...
double& matrice_nonsym::operator()(int i, int j) //Lecture et écriture
{
    ...
    else if (i>j) //triangle inférieur
    {
        if (j>=Profil[i]) //après le premier terme non-nul de la ligne
        {
            return Lower[Posdiag[i-1]+(j-Profil[i]+1)];
...
matrice_nonsym& matrice_nonsym::operator+=(const matrice_nonsym& A)
{
    ...
    for (int i=0; i<n; ++i)
    {
        (*this)(i,i)+=A(i,i); //n'incrémente que dans le upper
...
matrice_nonsym LUdecomposition(const matrice_nonsym& A)
{
    ...
    for (int p=1; p<=n; p++)
    {
        ...
        {
            //cout<<"Boucle en i:"<<i<<endl;
            if (p-1>=lu.Profil[i])
            {
                //cout<<"Condition-2 sur p:"<<p<<endl;
                lu(i,p-1) = A(i,p-1);
                for (int k=lu.Profil[p-1]; k<=(p-1)-1; k++)
                {
                    //cout<<"Boucle en k:"<<k<<endl;
...
int Nbtri = Numtri.size();
    ...
    //Nombre de sommets: Nbsom

int Nbsom = Coorneu.size();
...
vecteur resolsys(const matrice_nonsym A, const vecteur b) //Ax=b
{
    ...
    for (int i=1;i<n+1;++i)
    {
        //cout<<"Boucle-1 sur i:"<<i<<endl;
        double S=b[n-1];
        for (int j=1;j<i;++j)
        {
            //cout<<"Boucle-1 sur j:"<<j<<endl;
            if (j-1>=lu.Profil[i-1])
            {
    ...
    for (int i=n;i>0;--i)
    {
        //cout<<"Boucle-2 sur i:"<<i<<endl;
        double SS=y[i-1];
        for (int j=n;j>i;--j)
        {
            //cout<<"Boucle-2 sur j:"<<j<<endl;
            if (i-1>=lu.Profil[j-1])
            {
...
{
    vecteur Q(sommets.size());
    vector<Point>::const_iterator it;
    it = sommets.begin();
    for (int i=0;i<sommets.size();++i,++it)
    {
        Point P=(*it);
        Q[i]=pos(P.x+P.y-K);
    }
